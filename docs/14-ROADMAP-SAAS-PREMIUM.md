...
| E2 | Tests entreprise : intégration réelle PostgreSQL/MinIO, priorité aux modules financiers critiques, matrice Playwright multi-navigateurs/mobile, régression visuelle | **Partiel** (cette itération) — `docs/20-TESTS-ENTREPRISE.md` : modules `financings` et `committee` couverts contre un vrai PostgreSQL (verrouillage de lignes, contrainte unique et `UPDATE` conditionnel, double-soumission concurrente) ; MinIO, les modules critiques restants (administration, partenaires) et la matrice Playwright restent à faire |
