...
| E7 | Architecture de production OCI/Kubernetes | **Partiel avancé** — images non-root avec healthchecks, profils Compose, scans Trivy, SBOM CycloneDX signés en mode keyless Sigstore et cible OCI/Kubernetes documentée (`docs/24-DEPLOIEMENT-PRODUCTION-OCI-K8S.md`, `k8s/`). Durcissement livré : probes liveness/readiness séparées avec contrôle PostgreSQL/S3, Pod Security `restricted`, seccomp, capabilities supprimées, jetons de ServiceAccount désactivés, `NetworkPolicy` en refus entrant par défaut, `PodDisruptionBudget`, rolling update sans indisponibilité et invariants de la base Kustomize contrôlés en CI. Restent : registre OCI institutionnel, gestionnaire de secrets réel, adaptations REC/PPD/PROD, politique egress liée aux destinations retenues et validation sur le cluster cible. |
