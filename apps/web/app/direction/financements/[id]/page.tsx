...
export default function FinancingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className={portal.main}>
    ...
    <div className={styles.detailGrid}>
      <section className={`${portal.card} ${styles.panel}`}><h2>Décaissements</h2><form className={styles.review} onSubmit={planDisbursement}><div className={portal.field}><label>Montant à planifier — disponible {amount(Math.max(0, remainingDisbursement))} GNF</label><input type="number" min="1" max={Math.max(0, remainingDisbursement)} required value={disbursementAmount} onChange={(event) => setDisbursementAmount(event.target.value)} /></div><div className={portal.field}><label>Date prévue</label><input type="date" required value={plannedDate} onChange={(event) => setPlannedDate(event.target.value)} /></div><button className={portal.primary} disabled={remainingDisbursement <= 0}>Planifier</button></form></section>
    ...
    <section className={`${portal.card} ${portal.formCard} ${portal.section}`}><h2>Snapshot d’impact</h2><p className={portal.lead}>Une nouvelle saisie sur la même période remplace le snapshot précédent et conserve une trace d’audit.</p><form onSubmit={saveImpact}><div className={portal.formGrid}><div className={portal.field}><label>Période</label><input type="date" required value={impactPeriod} onChange={(event) => setImpactPeriod(event.target.value)} /></div><div className={portal.field}><label>Chiffre d’affaires (GNF)</label><input type="number" min="0" value={revenue} onChange={(event) => setRevenue(event.target.value)} /></div><div className={portal.field}><label>Employés</label><input type="number" min="0" value={employees} onChange={(event) => setEmployees(event.target.value)} /></div><div className={portal.field}><label>Femmes</label><input type="number" min="0" value={women} onChange={(event) => setWomen(event.target.value)} /></div><div className={portal.field}><label>Hommes</label><input type="number" min="0" value={men} onChange={(event) => setMen(event.target.value)} /></div><div className={portal.field}><label>Jeunes</label><input type="number" min="0" value={young} onChange={(event) => setYoung(event.target.value)} /></div><div className={portal.field}><label>Emplois créés</label><input type="number" min="0" value={jobsCreated} onChange={(event) => setJobsCreated(event.target.value)} /></div><div className={portal.field}><label>Emplois maintenus</label><input type="number" min="0" value={jobsMaintained} onChange={(event) => setJobsMaintained(event.target.value)} /></div></div><div className={portal.buttonRow}><button className={portal.primary}>Enregistrer l’impact</button></div></form></section>
