...
type User = { id: string; email: string; nom: string; prenom?: string; actif: boolean; mfaRequired: boolean; roles: string[]; entrepriseId?: string | null; raisonSociale?: string; lastLoginAt?: string | null };

const emptyForm = { email: '', nom: '', prenom: '', password: '', roles: ['AGENT_FODIP'], entrepriseId: '', mfaRequired: false };

export default function UsersAdministrationPage() {
  const [users, setUsers] = useState<User[]>([]); const [roles, setRoles] = useState<Role[]>([]); const [enterprises, setEnterprises] = useState<Enterprise[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const [usersBody, rolesBody, enterprisesBody] = await Promise.all([usersResponse.json(), rolesResponse.json(), enterprisesResponse.json()]);
    if (!usersResponse.ok) throw new Error(usersBody.message ?? 'Session administrateur requise');
    if (!rolesResponse.ok || !enterprisesResponse.ok) throw new Error('Référentiels indisponibles');
    setUsers(usersBody.items ?? []); setRoles(rolesBody.items ?? []); setEnterprises(enterprisesBody.items ?? []);
  }, [search]);
  ...
  async function create(event: FormEvent) {
    ...
    const response = await fetch('/api/administration/users', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ...form, entrepriseId: form.entrepriseId || undefined }) });
  ...
  async function save(user: User) {
    setMessage('');
    const response = await fetch(`/api/administration/users/${user.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ actif: user.actif, mfaRequired: user.mfaRequired, roles: user.roles, entrepriseId: user.entrepriseId || null }) });
    const body = await response.json().catch(() => ({}));
  ...
  return <main className={portal.main}><p className={portal.eyebrow}>Super administration</p><h1 className={portal.title}>Utilisateurs et rôles</h1>
    ...
    <section className={`${portal.card} ${portal.formCard} ${portal.section}`}><div className={portal.sectionHeader}><div><h2>Créer un utilisateur</h2><p>Le mot de passe initial respecte la politique forte et n’est jamais journalisé.</p></div></div>
      <form onSubmit={create}><div className={portal.formGrid}>
        ...
        <div className={portal.field}><label htmlFor="role">Rôle</label><select id="role" value={form.roles[0]} onChange={(event) => setForm({ ...form, roles: [event.target.value], entrepriseId: event.target.value === 'PME' ? form.entrepriseId : '' })}>{roles.map((role) => <option key={role.code} value={role.code}>{role.nom}</option>)}</select></div>
    ...
    <section className={`${portal.card} ${portal.tableCard} ${portal.section}`}><div className={portal.sectionHeader}><div><h2>Comptes existants</h2><p>{users.length} compte{users.length === 1 ? '' : 's'} dans le périmètre.</p></div><div className={portal.field}><label htmlFor="search">Recherche</label><input id="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nom ou email" /></div></div>
      <div style={{ overflowX: 'auto' }}><table className={portal.table}><thead><tr><th>Utilisateur</th><th>Rôles</th><th>Entreprise PME</th><th>Actif</th><th>MFA exigé</th><th>Action</th></tr></thead><tbody>{users.map((user) => <tr key={user.id}>
