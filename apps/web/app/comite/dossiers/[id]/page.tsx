...
export default function CommitteeApplicationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className={portal.main}>
    ...
    {dossier.score && <section className={`${portal.card} ${portal.tableCard} ${portal.section}`} tabIndex={0} role="region" aria-label="Tableau, défilement horizontal sur petit écran"><table className={portal.table}><thead><tr><th>Critère</th><th>Note</th><th>Contribution</th><th>Justification</th></tr></thead><tbody>{dossier.score.criteres.map((item) => <tr key={item.code}><td>{item.libelle}</td><td>{item.scoreObtenu}/{item.scoreMax}</td><td>{Number(item.contribution).toFixed(2)}</td><td>{item.commentaire || '—'}</td></tr>)}</tbody></table></section>}
    <section className={`${portal.card} ${portal.formCard} ${portal.section}`}><h2>Décision humaine</h2>{dossier.statut === 'PRET_COMITE' ? <form className={`${styles.review} ${portal.section}`} onSubmit={submit}><label htmlFor="decision">Décision</label><select id="decision" value={decision} onChange={(event) => setDecision(event.target.value)}><option value="APPROUVE">Approuver</option><option value="COMPLEMENT_REQUIS">Demander un complément</option><option value="REJETE">Rejeter</option></select>{decision === 'APPROUVE' && <div className={portal.formGrid}><div className={portal.field}><label htmlFor="montantApprouve">Montant approuvé (GNF)</label><input id="montantApprouve" type="number" min="1" max={Number(dossier.montantDemande)} required value={montant} onChange={(event) => setMontant(event.target.value)} /></div><div className={portal.field}><label htmlFor="dureeMois">Durée (mois)</label><input id="dureeMois" type="number" min="1" max="120" required value={duree} onChange={(event) => setDuree(event.target.value)} /></div><div className={portal.field}><label htmlFor="tauxInteret">Taux d’intérêt (%)</label><input id="tauxInteret" type="number" min="0" max="100" step="0.0001" value={taux} onChange={(event) => setTaux(event.target.value)} /></div><div className={portal.field}><label htmlFor="differeMois">Différé (mois)</label><input id="differeMois" type="number" min="0" max={Number(duree) || 120} value={differe} onChange={(event) => setDiffere(event.target.value)} /></div></div>}<label htmlFor="decisionCommentaire">Motivation, conditions ou réserves du comité</label><textarea id="decisionCommentaire" required={decision !== 'APPROUVE'} minLength={decision === 'APPROUVE' ? undefined : 3} value={commentaire} onChange={(event) => setCommentaire(event.target.value)} placeholder="Motivation, conditions ou réserves du comité" /><button className={portal.primary}>Enregistrer la décision</button></form> : <p className={portal.lead}>Ce dossier a déjà quitté la file décisionnelle.</p>}</section>
  </main>;
}
