# Étape 14 — Feuille de route institutionnelle nationale

## Objectif

Faire passer FODIP Digital 2030 de son socle fonctionnel initial à une plateforme effectivement
opérée, homologuée et gouvernée comme un service public numérique institutionnel national :
identité visuelle soignée, fiabilité opérationnelle mesurable, conformité et sécurité de niveau
étatique. Ce document découpe l'ambition en cinq axes indépendants, chacun en phases livrables
séparément, pour permettre un avancement continu sans big-bang.

Le statut officiel, les autorités de validation et les portes de mise en production sont définis
dans `docs/26-CADRE-INSTITUTIONNEL.md`. Cette roadmap décrit les travaux ; elle ne remplace jamais
...
| Phase | Contenu | Statut |
|---|---|---|
| B1 | RBAC fin, JWT, hachage bcrypt, isolation multi-tenant PME testée en e2e | Fait (socle initial) |
...
| B9 | Rendre fonctionnels tous les rôles prévus dans `docs/01-SOCLE-FONCTIONNEL-INITIAL.md` — `AUDITEUR` avait des permissions RBAC en base depuis le début (`audit.read`, `financing.read`, `impact.read`) mais aucune route API ne les vérifiait ni aucun portail web ne les exploitait ; `PARTENAIRE_BANCAIRE` restait sans surface API (voir axe D1) | **Fait** (cette itération, en deux temps : AUDITEUR puis PARTENAIRE_BANCAIRE via D1) |
...
## Axe E — Fondations d'une plateforme institutionnelle critique
...
niveau attendu d'un service public numérique critique, en petits lots indépendants (une PR par lot,
...
| E3 | Refonte UI/UX ultra premium : design system partagé, navigation mobile, états de chargement/erreur/vide systématiques, assistants PME/Agent/Comité/Direction | **Partiel** (mission « présentation Directeur général », PR #51-52 + lot UI institutionnel partagé) — `docs/23-PRESENTATION-DIRECTION-GENERALE.md` et `docs/31-UI-INSTITUTIONNEL-PARTAGE.md` : navigation mobile corrigée (menu hamburger + tiroir accessible, `AppShell` partagé sur les 7 portails, fonctionnel dès 360px) ; composants mutualisés (`AppShell`, `AccountMenu`, `Drawer`, `KpiCard`, `ExecutiveAlert`, `ConfirmDialog`, `Skeleton`, `EmptyState`, `ErrorState`, `Button`, `Toast`, `Dialog`, `FilterBar`, `Breadcrumbs`, `ResponsiveTable`) ; `loading.tsx`/`error.tsx`/`not-found.tsx` sur tous les segments de portail ; `window.prompt`/`window.confirm` éliminés ; première adoption des nouveaux composants sur `/direction/financements`, `/agent/dossiers` et `/entrepreneur/suivi`, avec tests dédiés 360/390/768/1024/1440 px. Reste : aucun « assistant » guidé (wizard) construit pour PME/Agent/Comité/Direction et migration progressive des autres écrans vers les composants partagés, sans big-bang |
...
| E5 | Intégrité financière : idempotency keys, maker-checker, rapprochement bancaire, verrouillage optimiste, contraintes PostgreSQL sur montants/statuts | **Partiel** (PR #53 + lot rapprochement bancaire) — idempotence, maker-checker sur les décaissements et verrouillage optimiste de la fiche entreprise déjà livrés. Le rapprochement bancaire est maintenant opérationnel (`database/022_bank_reconciliation.sql`, `docs/25-RAPPROCHEMENT-BANCAIRE.md`) : mouvements de relevé séparés des écritures internes, rapprochement transactionnel exact banque/sens/montant, unicité empêchant tout double rapprochement, permissions dédiées, audit, tests réels PostgreSQL et écran Direction. Restent hors périmètre jusqu'à validation comptable : import CSV spécifique à chaque banque, multi-devises, frais/écarts justifiés et rapprochement plusieurs-à-plusieurs ; étendre le maker-checker si d'autres opérations sensibles gagnent un flux à deux étapes. |
...
| E7 | Architecture de production OCI/Kubernetes | **Partiel avancé** — images non-root avec healthchecks, scans Trivy, SBOM CycloneDX signés en CI et cible Kubernetes durcie (`docs/24-DEPLOIEMENT-PRODUCTION-OCI-K8S.md`, `k8s/`). La chaîne de release OCI (`docs/29-CHAINE-RELEASE-OCI.md`) publie dans GHCR uniquement un tag SemVer issu de `main` après CI/CodeQL, attache SBOM et provenance, signe les digests avec Sigstore et produit un manifeste de release signé. Restent : décision/paramétrage du registre institutionnel définitif, gestionnaire de secrets réel, adaptations REC/PPD/PROD, politique egress liée aux destinations retenues et validation sur le cluster cible. |
