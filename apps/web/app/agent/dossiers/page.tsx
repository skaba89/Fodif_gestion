...
export default function AgentDossiersPage() {
  ...
  const load = useCallback(async (status = statut, search = recherche) => {
  ...
  function filter(event: FormEvent) { event.preventDefault(); setMessage(''); load().catch((error) => setMessage(error.message)); }
  ...
  return <main className={portal.main}>
    ...
    <section className={styles.metrics}>
      ...
      <article className={`${portal.card} ${styles.metric}`}><strong>{counts.SOUMIS ?? 0}</strong><span>À prendre en charge</span></article>
      <article className={`${portal.card} ${styles.metric}`}><strong>{counts.EN_INSTRUCTION ?? 0}</strong><span>En instruction</span></article>
      <article className={`${portal.card} ${styles.metric}`}><strong>{counts.PRET_COMITE ?? 0}</strong><span>Prêts pour comité</span></article>
    </section>
