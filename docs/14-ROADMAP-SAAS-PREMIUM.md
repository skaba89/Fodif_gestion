...
| E2 | Tests entreprise : intégration réelle PostgreSQL/MinIO, priorité aux modules financiers critiques, matrice Playwright multi-navigateurs/mobile, régression visuelle | **Partiel** (cette itération, complété par la mission « présentation Directeur général », PR #51-52) — `docs/20-TESTS-ENTREPRISE.md` : les cinq modules métier critiques (`financings`, `committee`, `administration`, `partner`, `documents`) couverts contre un vrai PostgreSQL (verrouillage de lignes, contrainte unique, `UPDATE` conditionnel, verrou consultatif global, isolation entre banques partenaires, double-soumission concurrente) et, pour `documents`, un vrai stockage compatible S3 (round-trip d'octets, contrôle d'intégrité par checksum, isolation PME) ; matrice Playwright desktop (Chromium/Firefox/WebKit) faite ; son volet mobile (Pixel 7 Chromium vérifié, iPhone 14 WebKit configuré) est maintenant activé, le bug de navigation qui le bloquait étant corrigé par E3 ; reste : régression visuelle (captures de référence non ajoutées — un environnement de développement sans accès au runner CI cible produirait des captures qui casseraient au premier run CI plutôt que de protéger contre une vraie régression, voir `docs/23-PRESENTATION-DIRECTION-GENERALE.md` §8) |
| E3 | Refonte UI/UX ultra premium : design system partagé, navigation mobile, états de chargement/erreur/vide systématiques, assistants PME/Agent/Comité/Direction | **Partiel** (mission « présentation Directeur général », PR #51-52) — `docs/23-PRESENTATION-DIRECTION-GENERALE.md` : navigation mobile corrigée (menu hamburger + tiroir accessible, `AppShell` partagé sur les 7 portails, fonctionnel dès 360px) ; composants mutualisés (`AppShell`, `AccountMenu`, `Drawer`, `KpiCard`, `ExecutiveAlert`, `ConfirmDialog`, `Skeleton`, `EmptyState`, `ErrorState`) ; `loading.tsx`/`error.tsx`/`not-found.tsx` sur tous les segments de portail ; `window.prompt`/`window.confirm` éliminés. Reste : composants `Toast`/`Button`/`Dialog` génériques, `FilterBar` autonome, `Breadcrumbs`, `ResponsiveTable` (tableaux en cartes sur mobile — un correctif d'accessibilité minimal est en place en attendant, voir E2) ; aucun « assistant » guidé (wizard) construit pour aucun portail |
| E4 | Identité et sécurité entreprise : cycle de vie de session complet, révocation, rate limiting distribué, durcissement OIDC, séparation et rotation des clés | **Partiel** (cette itération) — révocation de session : jusqu'ici, « Déconnexion » n'effaçait que le cookie du navigateur, le jeton JWT restant valide côté API jusqu'à son expiration naturelle (15 minutes par défaut). Chaque jeton porte désormais un identifiant unique (`jti`) ; `POST /auth/logout` le révoque immédiatement (table `revoked_tokens`, vérifiée par `JwtAuthGuard` à chaque requête), sans affecter les autres sessions actives du même compte. Couvre les trois chemins d'émission de jeton (connexion directe, MFA, OIDC), qui passent tous par `SessionTokenService`. Rate limiting distribué : `ThrottlerModule` utilisait le stockage en mémoire par défaut de `@nestjs/throttler` (une `Map` par processus) — avec plusieurs instances de l'API derrière un même load balancer, chaque instance comptait pour son propre compte, rendant la limite réelle égale à N x nombre d'instances, et une protection comme celle sur `/auth/login` (bourrage d'identifiants) contournable en répartissant les tentatives. `PostgresThrottlerStorageService` (`apps/api/src/common/postgres-throttler-storage.service.ts`) remplace ce stockage par PostgreSQL — la seule dépendance déjà partagée par toutes les instances, sans introduire Redis ni aucun autre composant. Une première version (fenêtre fixe, un compteur agrégé par clé) passait tous les tests locaux mais a fait échouer la CI réelle (`company-profile.spec.ts` sur les projets `Pixel 7`/`iPhone 14`) : `playwright.config.ts` (commentaire `HEAVY_LOGIN_SPECS`) compte explicitement sur la vraie décroissance glissante du stockage en mémoire d'origine pour que des connexions espacées dans le temps ne s'accumulent jamais dans une même fenêtre — une fenêtre fixe est une garantie différente, plus stricte, pas une réimplémentation équivalente. Cause racine trouvée, pas supposée, et corrigée : `rate_limit_hits` stocke désormais une ligne par requête comptée (pas un compteur agrégé), chacune vivant exactement `ttl` après elle-même — une vraie fenêtre glissante — et `rate_limit_blocks` porte l'état « actuellement bloqué » séparément ; sécurité de concurrence via un verrou consultatif PostgreSQL transactionnel (`pg_advisory_xact_lock`, scopé à la clé) plutôt qu'un verrou de ligne, puisqu'il n'existe plus une seule ligne représentant l'état d'une clé. Comme `ThrottlerGuard` est un garde global (toutes les routes), une panne de la base ouvre plutôt que de fermer (requête autorisée, avertissement journalisé) — un contrôle de défense en profondeur qui échoue fermé transformerait un incident PostgreSQL passager en panne totale de toute l'API, sur toutes les routes ; comportement exercé réellement par `test/app.e2e-spec.ts`, qui démarre toute `AppModule` (ce garde inclus) sans base configurée. Vérifié par 8 tests unitaires (logique de fenêtre glissante/blocage/échec ouvert contre un client simulé, décroissance prouvée avec des timers Jest simulés) et 6 tests d'intégration contre un vrai PostgreSQL (état partagé entre deux instances distinctes du service — le point même de cet axe —, incréments concurrents réels sans perte via le verrou consultatif réel, blocage sous concurrence réelle, lignes indépendantes par `(clé, throttler)`, déblocage après expiration réelle du blocage, décroissance glissante réelle contre de vrais timestamps PostgreSQL), puis reproduction réelle de l'échec CI initial sur une vraie stack locale (`company-profile.spec.ts` sur `chromium`/`Pixel 7`, seul puis après avoir rejoué les mêmes specs que le job CI qui échouait) confirmant la correction. Rotation des clés (signature JWT) : `JwtAuthGuard` vérifiait toujours contre le seul `JWT_SECRET` courant — le faire tourner sans invalider en bloc toutes les sessions actives n'était pas possible. `JwtKeyResolverService` (`apps/api/src/auth/jwt-key-resolver.service.ts`) tague chaque jeton signé avec un `kid` (les 8 premiers caractères hex du SHA-256 du secret qui l'a signé — déterministe, donc rien à gérer à la main) ; un `JWT_SECRET_PREVIOUS` optionnel reste accepté en vérification pendant la fenêtre de rotation (ce dépôt n'a pas de jetons de rafraîchissement, donc `JWT_ACCESS_TTL`, 15 minutes par défaut, borne cette fenêtre). Scopé à la signature JWT seule, pas aux secrets dérivés via `deriveSecret()` (chiffrement PII au repos, secrets TOTP MFA, jetons de flux/livraison OIDC) — les faire tourner casserait le déchiffrement de tout ce qui est déjà chiffré avec l'ancienne clé dérivée sans un chiffrement versionné (kid attaché à chaque donnée chiffrée, migration progressive), qui n'est pas livré ici et reste signalé comme limitation plutôt que résolu à moitié en silence. Vérifié par 5 tests unitaires (`security-policy.test.cjs`, dérivation déterministe du kid, jamais de collision qui écraserait la clé courante), 4 tests unitaires (`JwtKeyResolverService`), 3 tests unitaires (`JwtAuthGuard`, lecture du kid, repli sur la clé courante pour un jeton sans kid, échec propre sur un jeton illisible), et 4 tests de bout en bout contre l'application réelle complète (`jwt-key-rotation.e2e-spec.ts`) : un jeton signé avec l'ancien secret passe pendant la fenêtre de rotation, un jeton signé avec le nouveau passe aussi, l'ancien est rejeté une fois `JWT_SECRET_PREVIOUS` retiré, et un jeton « historique » sans aucun kid (émis avant cet axe) reste accepté. Durcissement OIDC : le jeton de livraison (`OidcService#issueDeliveryToken`, remis au navigateur via une redirection `?oidc_token=...` — un canal exposé : historique du navigateur, journaux d'accès serveur/proxy) n'était vérifié que par signature et expiration (2 minutes), sans jamais suivre s'il avait déjà été consommé — quiconque en obtenait une copie pendant sa fenêtre pouvait appeler `/auth/oidc/exchange` à son tour et obtenir une session à la place de la victime. `oidc_delivery_tokens_used` (`database/021_oidc_delivery_token_replay_protection.sql`) réclame désormais le `jti` du jeton au premier échange réussi (`INSERT ... ON CONFLICT DO NOTHING`, même forme que `revoked_tokens`) — une deuxième tentative, même dans la fenêtre de validité, est rejetée. Vérifié par 5 tests unitaires (`oidc.service.spec.ts`, dont le rejet d'un deuxième échange) et 4 tests d'intégration contre un vrai PostgreSQL, dont une vraie course de concurrence (`Promise.allSettled` sur trois échanges simultanés du même jeton — exactement un réussit, la contrainte `UNIQUE` réelle tranche, jamais supposée suffisante sans preuve). Reste : rotation des secrets dérivés (PII/MFA/OIDC) — nécessite un chiffrement versionné, non entamé |
| E5 | Intégrité financière : idempotency keys, maker-checker, rapprochement bancaire, verrouillage optimiste, contraintes PostgreSQL sur montants/statuts | **Partiel** (PR #53, cette itération) — clé d'idempotence (`Idempotency-Key`, table `idempotency_keys`) sur les 4 écritures financières à risque de doublon réel (décaissement/remboursement, Direction et Partenaire), avec claim atomique (`INSERT ... ON CONFLICT DO NOTHING`) prouvé par un vrai test de concurrence contre PostgreSQL, rejeu de la réponse stockée sur même clé, 409 sur même clé + charge utile différente ; contraintes `CHECK` ajoutées sur `decaissements.statut`/`echeances.statut`. Maker-checker sur l'exécution des décaissements (le seul flux financier à deux étapes planification/exécution du dépôt) : la personne qui exécute (`decaissements.executed_by`) ne peut pas être celle qui a planifié (`created_by`), imposé à la fois par `FinancingsService` (403 explicite) et par une contrainte `CHECK` PostgreSQL (`ck_decaissements_maker_checker`, vérifiée directement au niveau base par un test qui contourne l'application). Verrouillage optimiste sur la fiche entreprise (`entreprises.version`, `PATCH /companies/me`) : la seule page « charger, éditer sur place, enregistrer » de ce dépôt avec une vraie interface web l'exerçant — un formulaire chargé pendant qu'un autre utilisateur de la même PME (ou un second onglet) enregistre entre-temps est refusé (409), jamais silencieusement écrasé, prouvé par un vrai test de concurrence contre PostgreSQL et par un scénario Playwright à deux sessions réelles. `applications.repository.ts#updateOwned` (brouillon de dossier de financement) a la même lacune, non corrigée ici faute de page web l'exerçant aujourd'hui — à traiter si/quand une telle page existe. Reste : rapprochement bancaire, et étendre le maker-checker au-delà du décaissement si d'autres opérations sensibles gagnent un flux à deux étapes |
| E6 | Documents entreprise : antivirus, quarantaine, checksum, versioning, upload en streaming | **Partiel** (cette itération) — scan antivirus réel à l'upload (`ClamAvService`, protocole `INSTREAM` de `clamd` implémenté directement en TCP, sans dépendance) : désactivé par défaut (`CLAMAV_HOST` non défini, comme OIDC), et une fois activé, tout échec du scan (démon injoignable, erreur du protocole) fait échouer l'upload plutôt que de le laisser passer sans scan (503, jamais un bypass silencieux) ; vérifié par un vrai serveur TCP de test reproduisant le protocole `clamd` et le vecteur de test standard EICAR (signature détectée, faux positif inoffensif). Service Docker Compose optionnel (profil `antivirus`), non vérifié par un vrai `docker compose up` dans cet environnement (accès Docker Hub bloqué en sandbox, comme documenté pour `docker-compose.presentation.yml`) — vérification réelle laissée à la CI. Versioning : un bug réel a été trouvé et corrigé au passage — l'écran de dossier de l'agent (`agent-applications.repository.ts`) et celui du comité (`committee.repository.ts`) affichaient chacun leur propre requête directe sur `dossier_documents`, sans jamais exclure les versions remplacées ; un agent pouvait donc valider/rejeter un document que la PME avait déjà corrigé, pendant que la nouvelle version restait invisible. Corrigé par une colonne `superseded_by` (auto-référence, `database/019_document_versioning.sql`, index partiel sur les documents courants) : chaque nouvel upload du même type dans le même dossier marque l'ancien comme remplacé dans la même transaction, sans jamais l'effacer (historique complet conservé) ; `verify()` verrouille la ligne (`FOR UPDATE`) avant de décider et renvoie un résultat typé (`NOT_FOUND` / `SUPERSEDED` / `OK`) traduit en 404/409/200 — un agent qui tente de vérifier un document déjà remplacé reçoit un 409 explicite, jamais un succès silencieux sur la mauvaise version. Les trois requêtes directes sur `dossier_documents` (`documents.repository.ts`, `agent-applications.repository.ts`, `committee.repository.ts`) filtrent désormais `superseded_by IS NULL`, sauf `listOwn` (PME) qui garde volontairement tout l'historique — le portail entrepreneur affiche un badge « Remplacé » sur les versions dépassées. Vérifié par 6 tests d'intégration contre un vrai PostgreSQL (chaîne de remplacement, portée au bon type/dossier, exclusion de `listForReview`, 409 sur l'ancienne version, succès sur la version courante, historique complet toujours visible côté PME) en plus des tests unitaires du service. Reste : checksum déjà couvert par ailleurs (SHA-256 à l'upload et au téléchargement, voir `documents.service.ts`), quarantaine, upload en streaming (le fichier est toujours bufferisé en mémoire) |
| E7 | Architecture Docker entreprise : profils dédiés, images non-root avec healthchecks, SBOM signé, cible de production OCI/K8s documentée sans dépendance cloud | **Partiel** (cette itération) — images non-root : déjà en place avant cette itération (`USER node` dans les deux Dockerfiles), non modifié. Healthchecks : désormais définis dans les images elles-mêmes (`HEALTHCHECK` dans `apps/api/Dockerfile` et `apps/web/Dockerfile`, ce dernier n'en avait aucun) plutôt que seulement dans `docker-compose.yml` — fonctionnent donc aussi pour un `docker run` autonome, la colonne STATUS de `docker ps`, ou une sonde de disponibilité Kubernetes, qu'aucun de ces contextes ne lit `docker-compose.yml`. Profils dédiés : déjà en place avant cette itération (`observability`, `antivirus`), non modifié ici. SBOM signé : chaque SBOM CycloneDX (déjà généré en CI via `anchore/sbom-action`) est désormais signé par `cosign sign-blob` en mode keyless Sigstore (`.github/workflows/ci.yml`, job `docker`) — le job échange son jeton OIDC GitHub Actions (`permissions.id-token: write`, scopé à ce seul job) contre un certificat Fulcio de courte durée lié à l'identité exacte du run, sans jamais générer ni stocker de clé privée ; l'enveloppe de signature (`*.sigstore.json`, signature + certificat + preuve Rekor) est uploadée à côté du SBOM. Voir `docs/19-GOUVERNANCE-SUPPLY-CHAIN.md` pour la commande de vérification côté consommateur. Cible de production OCI/K8s : documentée dans `docs/24-DEPLOIEMENT-PRODUCTION-OCI-K8S.md`, avec des manifestes Kubernetes réels (`k8s/`) — namespace, configuration, gabarit de secrets, `Job` de migration réutilisant `scripts/run-migrations.js`, déploiements API/web à deux réplicas (sûr désormais grâce au rate limiting et à la révocation de session portés par PostgreSQL, axe E4) avec des sondes de disponibilité reprenant exactement les routes du `HEALTHCHECK` de chaque image, gabarit d'Ingress — sans dépendance à un fournisseur cloud particulier. Vérifié par `kubeconform` (schéma Kubernetes réel, 9 ressources, zéro erreur) ; non vérifié contre un vrai cluster (aucun Kubernetes disponible dans ce bac à sable), signalé explicitement dans le document plutôt que supposé. Reste : chart Helm ou configuration Kustomize (les manifestes bruts suffisent pour un premier déploiement), `NetworkPolicy` restreignant le trafic entre pods, intégration à un gestionnaire de secrets réel (le gabarit `k8s/02-secret.example.yaml` reste le minimum qui fonctionne partout, pas la fin visée) |
