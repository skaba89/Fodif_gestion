...
| E4 | Identité et sécurité entreprise : cycle de vie de session complet, révocation, rate limiting distribué, durcissement OIDC, séparation et rotation des clés | **Partiel** (cette itération) — révocation de session : jusqu'ici, « Déconnexion » n'effaçait que le cookie du navigateur, le jeton JWT restant valide côté API jusqu'à son expiration naturelle (15 minutes par défaut). Chaque jeton porte désormais un identifiant unique (`jti`) ; `POST /auth/logout` le révoque immédiatement (table `revoked_tokens`, vérifiée par `JwtAuthGuard` à chaque requête), sans affecter les autres sessions actives du même compte. Couvre les trois chemins d'émission de jeton (connexion directe, MFA, OIDC), qui passent tous par `SessionTokenService`. Reste : rate limiting distribué (l'actuel est en mémoire, par instance), durcissement OIDC, séparation et rotation des clés |
