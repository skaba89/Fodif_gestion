...
| E4 | Identité et sécurité entreprise : cycle de vie de session complet, révocation, rate limiting distribué, durcissement OIDC, séparation et rotation des clés | **Partiel** (cette itération) — révocation de session : jusqu'ici, « Déconnexion » n'effaçait que le cookie du navigateur, le jeton JWT restant valide côté API jusqu'à son expiration naturelle (15 minutes par défaut). Chaque jeton porte désormais un identifiant unique (`jti`) ; `POST /auth/logout` le révoque immédiatement (table `revoked_tokens`, vérifiée par `JwtAuthGuard` à chaque requête), sans affecter les autres sessions actives du même compte. Couvre les trois chemins d'émission de jeton (connexion directe, MFA, OIDC), qui passent tous par `SessionTokenService`. Rate limiting distribué : `ThrottlerModule` utilisait le stockage en mémoire par défaut de `@nestjs/throttler` (une `Map` par processus) — avec plusieurs instances de l'API derrière un même load balancer, chaque instance comptait pour son propre compte, rendant la limite réelle égale à N x nombre d'instances, et une protection comme celle sur `/auth/login` (bourrage d'identifiants) contournable en répartissant les tentatives. `PostgresThrottlerStorageService` (`apps/api/src/common/postgres-throttler-storage.service.ts`) remplace ce stockage par PostgreSQL — la seule dépendance déjà partagée par toutes les instances, sans introduire Redis ni aucun autre composant. Fenêtre fixe avec blocage (compteur incrémenté par fenêtre, blocage une fois la limite dépassée), verrouillage `FOR UPDATE` pour la sécurité de concurrence (même schéma que le reste de ce dépôt) ; un `INSERT ... ON CONFLICT DO NOTHING` garantit qu'une ligne existe avant le verrouillage, faute de quoi deux premières requêtes concurrentes sur une même clé se verraient chacune attribuer le compte 1 (aucune ligne à verrouiller tant qu'aucune n'existe) — trouvé et corrigé par un vrai test de concurrence contre PostgreSQL, pas supposé correct. Comme `ThrottlerGuard` est un garde global (toutes les routes), une panne de la base ouvre plutôt que de fermer (requête autorisée, avertissement journalisé) — un contrôle de défense en profondeur qui échoue fermé transformerait un incident PostgreSQL passager en panne totale de toute l'API, sur toutes les routes ; comportement exercé réellement par `test/app.e2e-spec.ts`, qui démarre toute `AppModule` (ce garde inclus) sans base configurée. Vérifié par 7 tests unitaires (logique de fenêtre/blocage/échec ouvert contre un client simulé) et 5 tests d'intégration contre un vrai PostgreSQL (état partagé entre deux instances distinctes du service — le point même de cet axe —, incréments concurrents réels sans perte, blocage sous concurrence réelle, lignes indépendantes par `(clé, throttler)`, déblocage après expiration réelle du blocage), puis vérifié de bout en bout contre une vraie API démarrée localement : 5 tentatives de connexion échouées passent, la 6e et au-delà reçoivent 429, avec une vraie ligne PostgreSQL à l'appui. Reste : durcissement OIDC, séparation et rotation des clés |
