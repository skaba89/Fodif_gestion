...
| E2 | Tests entreprise : intégration réelle PostgreSQL/MinIO, priorité aux modules financiers critiques, matrice Playwright multi-navigateurs/mobile, régression visuelle | **Partiel** (cette itération) — `docs/20-TESTS-ENTREPRISE.md` : modules `financings`, `committee` et `administration` couverts contre un vrai PostgreSQL (verrouillage de lignes, contrainte unique, `UPDATE` conditionnel, verrou consultatif global, double-soumission concurrente) ; MinIO, le module `partner` restant et la matrice Playwright restent à faire |
